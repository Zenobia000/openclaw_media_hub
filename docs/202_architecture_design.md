...
- **L3 Component (組件圖)**:
  ...
  - **Config Manager**: 管理 `openclaw.json` 結構化 JSON（sections: meta, wizard, auth, agents, tools, commands, channels, gateway, plugins）與 `.env` 檔案（16+ 環境變數 upsert）。金鑰與設定值的讀寫、整合 `keyring` 安全儲存。Gateway token 產生與讀取邏輯（讀 openclaw.json > 讀 .env > `secrets.token_hex(32)` 產生）。
