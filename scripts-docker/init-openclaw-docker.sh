...
show_pairing_hint() {
    ...
    if [[ -n "$id" ]]; then
        ...
        printf '\033[33m  docker compose exec openclaw-gateway openclaw devices approve <ID>\033[0m\n'
    fi
}
...
if docker info >/dev/null 2>&1; then
    ...
    err "Docker 未啟動或未安裝，請先開啟 Docker Desktop。"
    exit 1
fi
...
# WSL2 DrvFs (9p) 的 mkdir 對隱藏目錄（.開頭）有快取不一致問題，
# 需透過 wslpath + cmd.exe 建立目錄作為 fallback。
ensure_dir() {
    local d="$1"
    [[ -d "$d" ]] && return 0
    mkdir -p "$d" 2>/dev/null && return 0
    # fallback：透過 Windows cmd.exe 建立（避免 DrvFs 9p 快取問題）
    local winpath
    winpath=$(wslpath -w "$d" 2>/dev/null) || return 1
    cmd.exe /c mkdir "$winpath" >/dev/null 2>&1 || true
    [[ -d "$d" ]]
}
...
if [[ -e "$OPENCLAW_DIR" && ! -d "$OPENCLAW_DIR" ]]; then
    ...
    rm -f "$OPENCLAW_DIR" 2>/dev/null || cmd.exe /c "del \"$(wslpath -w "$OPENCLAW_DIR")\"" >/dev/null 2>&1
fi
...
echo ""
info "啟動 Docker Compose..."
if docker compose up -d; then
    ok "Docker Compose 已啟動"
else
    err "Docker Compose 啟動失敗"
    exit 1
fi

echo ""
if ! wait_gateway "等待 Gateway 啟動"; then
    err "Gateway 未就緒，請檢查容器狀態。"
    exit 1
fi
...
if confirm_yes_no "是否要進行裝置配對？"; then
    ...
    request_id=""
    for ((w = 0; w < 60; w += 3)); do
        output=$(docker compose exec openclaw-gateway openclaw devices list 2>&1) || true
