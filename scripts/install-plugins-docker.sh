...
#
# 用法：./scripts/install-plugins-docker.sh
#
# 流程：
#   1. 安裝 LINE 插件
#   2. 安裝 Discord 插件
...
set_channel_config() {
    ...
    if ! command -v jq &>/dev/null; then
        err "需要 jq 工具來寫入設定。請先安裝：sudo apt install jq / brew install jq"
        return 1
    fi
...
install_plugin() {
    ...
    if ! confirm_yes_no "是否要安裝 $name 插件？"; then
        ...
        printf '\033[33m  docker compose exec openclaw-gateway openclaw plugins install %s\033[0m\n' "$package"
        return
    fi
    ...
    if [[ "$code" != "n" && "$code" != "N" ]]; then
        ...
        if invoke_gateway pairing approve "$channel" "$code"; then
            ...
            err "$name 配對失敗，請手動執行："
            printf '\033[33m  docker compose exec openclaw-gateway openclaw pairing approve %s <配對碼>\033[0m\n' "$channel"
        fi
    else
