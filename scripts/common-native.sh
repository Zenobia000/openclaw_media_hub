#!/usr/bin/env bash
# ============================================================
# common-native.sh — OpenClaw 腳本共用函式庫（Ubuntu 原生版）
#
# 用法：在腳本開頭加入
...
# ── 執行 Gateway CLI 指令（直接呼叫 openclaw）─────────────────
...
wait_gateway() {
    ...
    # 顯示服務狀態以協助除錯
    local svc_status
    svc_status=$(systemctl is-active "$SERVICE_NAME" 2>/dev/null) || true
    if [[ -n "$svc_status" ]]; then
        warn "服務狀態：$svc_status"
    fi
    return 1
}

# ── 重啟 openclaw-gateway 並等待就緒 ─────────────────────────
