...
```bash
python3 {skill_dir}/scripts/gmail_create_draft.py \
  --credentials "{skill_dir}/{credentials_file}" \
  --token "{skill_dir}/{token_file}" \
  --scopes "{gmail_scopes 以逗號分隔}" \
  --to "{from_email}" \
...
├── gmail_get_message.py    # 取得單封郵件詳情（補充上下文用）
└── gmail_create_draft.py   # 建立 Gmail 草稿
```

- Resend 發信透過 MCP Server（`resend/mcp-send-email`），無需自訂 script。
- `gmail_create_draft.py` 僅供 Gmail Draft 備選方案使用。
