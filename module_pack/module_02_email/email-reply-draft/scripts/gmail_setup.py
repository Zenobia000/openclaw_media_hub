...
用法：
    # 步驟 1：產生授權網址
    python3 gmail_setup.py --credentials client_secret.json --token token.json \
    ...
    python3 gmail_setup.py --credentials client_secret.json --token token.json \
        --scopes "https://www.googleapis.com/auth/gmail.readonly,https://www.googleapis.com/auth/gmail.compose" \
        --step exchange --code "AUTHORIZATION_CODE"
...
def step_exchange(credentials_path: str, token_path: str, scopes: list[str], code: str) -> dict:
    """用授權碼換取 token 並儲存。"""
    try:
        from google_auth_oauthlib.flow import InstalledAppFlow
    except ImportError:
...
def step_verify(credentials_path: str, token_path: str, scopes: list[str]) -> dict:
    """驗證 token 是否有效，嘗試讀取 Gmail 帳號資訊。"""
    try:
    ...
    try:
        ...
        return {
            ...
            "message": f"認證成功！已連接 Gmail：{profile.get('emailAddress', '')}",
        }
    except Exception as e:
        return {"ok": False, "error": f"無法存取 Gmail：{e}"}


def main():
    parser = argparse.ArgumentParser(description="Gmail OAuth2 初始化")
    ...
    parser.add_argument("--scopes", default=None,
                        help="Gmail API scopes（逗號分隔）。例如：https://www.googleapis.com/auth/gmail.readonly,https://www.googleapis.com/auth/gmail.compose")
    ...
    if args.step == "auth-url":
        ...
        if not args.code:
            ...
            result = step_exchange(args.credentials, args.token, scopes, args.code)
    elif args.step == "verify":
        result = step_verify(args.credentials, args.token, scopes)
    else:
