...
時間：2026-03-10 09:00
內容：以下任務將在 24 小時內到期：
  1. 完成課程網站首頁切版（截止：3/11 09:00）
  2. 提交行銷文案初稿（截止：3/11 12:00）
行動：請確認進度，任務清單 https://notion.so/tasks
```
...
時間：2026-03-10 08:01:23
結果：失敗（已重試 1 次）
錯誤：Web Search API 回傳 429 Too Many Requests
影響：今日早報未生成
行動：請檢查 API 額度，或稍後手動觸發重跑
```
